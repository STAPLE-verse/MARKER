...
| **STAPLE import** | `IMPORTED_STAPLE` | Copy the **selected `FormVersion` only** (not full history) into a new `MarkerForm` v1. Record `importedFromStapleFormId`/`VersionNumber`/`importedAt` (informational, no FK) + `originalImportHash` (fixity). | Same as native — a STAPLE draft has no external immutability guarantee. |
| **External import** | `IMPORTED_EXTERNAL` | CEDAR / RedCap / uploaded JSON. Capture `externalId` / `externalSourceUrl` + fixity hash + license at ingestion time. | Lighter "cataloging" path. If the source already has a stable identifier + license, do **not** force a redundant re-publish — record the relationship (`IsIdenticalTo` / `IsDerivedFrom`, DataCite `relatedIdentifiers`/`relationType` vocabulary) instead of minting a competing canonical PID for content that already has one. |
...
- Resolves the "Pending Decision" in [`forms-feature-plan.md`](./forms-feature-plan.md) §1: MARKER→STAPLE import will only ever happen via `PublishedSchema` (immutable, FAIR-complete) — Approach A, now decided rather than recommended. STAPLE never reads from `MarkerForm`/`MarkerFormVersion` directly.
